        ...
        <section className="space-y-5">
          <div className="rounded-xl border border-[#d2ccc1] bg-[#fbfaf7] p-5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-md border border-[#b8d1c4] bg-[#edf5f0] px-2.5 py-1 text-[9px] font-black uppercase tracking-wide text-[#246344]">{t(language, 'Nothing uploaded', 'कुछ अपलोड नहीं')}</span>
            </div>
          ...
          {!started ? (
            ...
            <>
              <div className={classes('rounded-[18px] border p-6 sm:p-8', assessment.outcome === 'danger' ? 'border-[#c87966] bg-[#fbefec]' : assessment.outcome === 'suspicious' ? 'border-[#d5bd78] bg-[#faf5e7]' : 'border-[#a9c9b7] bg-[#edf5f0]')}>
                <div className="flex items-start justify-between gap-4"><div><p className={classes('text-[10px] font-black uppercase tracking-[0.16em]', assessment.outcome === 'danger' ? 'text-[#9b3525]' : assessment.outcome === 'suspicious' ? 'text-[#795d18]' : 'text-[#286146]')}>{pick(language, assessment.eyebrow)}</p><h2 className="mt-3 text-3xl font-black tracking-[-0.045em]">{pick(language, assessment.headline)}</h2></div><span className={classes('rounded-md px-3 py-1.5 text-[9px] font-black uppercase tracking-wide text-white', assessment.outcome === 'danger' ? 'bg-[#a13d2a]' : assessment.outcome === 'suspicious' ? 'bg-[#8a681c]' : 'bg-[#315f78]')}>{assessment.outcome === 'danger' ? t(language, 'High risk', 'उच्च जोखिम') : assessment.outcome === 'suspicious' ? t(language, 'Verify first', 'पहले जाँचें') : t(language, 'Not authenticated', 'पहचान अपुष्ट')}</span></div>
